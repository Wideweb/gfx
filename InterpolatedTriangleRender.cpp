//
//  InterpolatedTriangleRender.cpp
//  hello-render
//
...
void InterpolatedTriangleRender::drawTriangles(std::vector<Vertex> vertexes, std::vector<int> indexes) {
...
std::vector<Vertex> InterpolatedTriangleRender::rasterizeTriange(Vertex v0, Vertex v1, Vertex v2) {
    ...
    auto degenerateTriangleByX = [&](Vertex top, Vertex middle, Vertex bottom) {
        ...
        for (float x = from.f0; x <= to.f0; x++) {
            out.push_back(interpolate(from, to, (x - from.f0) / (to.f0 - from.f0)));
        }
    };
    ...
    auto degenerateTriangleByY = [&](Vertex top, Vertex middle, Vertex bottom) {
        ...
        for (float y = from.f1; y <= to.f1; y++) {
            out.push_back(interpolate(from, to, (y - from.f1) / (to.f1 - from.f1)));
        }
    };
    
    auto horizontalUpwardTriangle = [&](Vertex top, Vertex middleLeft, Vertex middleRight) {
        for (float y = 0; y <= middleLeft.f1 - top.f1; y++) {
            
            Vertex from = interpolate(middleLeft, top, y / (middleLeft.f1 - top.f1));
            Vertex to = interpolate(middleRight, top, y / (middleRight.f1 - top.f1));
    ...
    auto horizontalDownwardTriangle = [&](Vertex top, Vertex middleLeft, Vertex middleRight) {
        for (float y = 0; y <= top.f1 - middleLeft.f1; y++) {
            
            Vertex from = interpolate(middleLeft, top, y / (top.f1 - middleLeft.f1));
            Vertex to = interpolate(middleRight, top, y / (top.f1 - middleRight.f1));
    ...
    std::sort(vertexes.begin(), vertexes.end(), [&](Vertex first, Vertex second) {
        return first.f1 < second.f1;
    });
    ...
    Vertex middle2 = interpolate(bottom, top, (bottom.f1 - middle.f1) / (bottom.f1 - top.f1));
...
Vertex InterpolatedTriangleRender::interpolate(Vertex v0, Vertex v1, float t) {
    return {
        interpolate(v0.f0, v1.f0, t),
        interpolate(v0.f1, v1.f1, t),
        interpolate(v0.f2, v1.f2, t),
        interpolate(v0.f3, v1.f3, t),
...
bool InterpolatedTriangleRender::isCollinear(Vertex v0, Vertex v1, Vertex v2) {
    return (v1.f0 - v0.f0) * (v2.f1 - v0.f1) == (v2.f0 - v0.f0) * (v1.f1 - v0.f1);
}
